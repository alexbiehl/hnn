...
updateNetwork :: (Floating (Vector a), Floating a, Product a, Storable a, Num (Vector a), Container Vector a) => a -> a -> ActivationFunction a -> ActivationFunctionDerivative a -> Network a -> Sample a -> Network a
updateNetwork alpha momentum act act' n@(Network{..}) (input, expectedOutput) = Network $ V.zipWith (+) corr_matrices corr
    where !xs = outputs n act input
          !ds = deltas n act' xs expectedOutput

          !corr = V.map (scale (-alpha)) ds
          !corr_matrices = V.map (scale momentum) matrices
...
backpropOnce :: (Floating (Vector a), Floating a, Product a, Num (Vector a), Container Vector a) => a -> a -> ActivationFunction a -> ActivationFunctionDerivative a -> Network a -> Samples a -> Network a
backpropOnce rate momentum act act' n samples = foldl' (updateNetwork rate momentum act act') n samples
{-# INLINE backpropOnce #-}
...
trainUntil :: (Floating (Vector a), Floating a, Product a, Num (Vector a), Container Vector a) => (Int -> Network a -> Samples a -> Bool) -> a -> a -> ActivationFunction a -> ActivationFunctionDerivative a -> Network a -> Samples a -> Network a
trainUntil pr learningRate momentum act act' net samples = go net 0
  where go n !k | pr k n samples = n
                | otherwise      = case backpropOnce learningRate momentum act act' n samples of
                                    n' -> go n' (k+1)
{-# INLINE trainUntil #-}
...
trainNTimes :: (Floating (Vector a), Floating a, Product a, Num (Vector a), Container Vector a) => Int -> a -> a -> ActivationFunction a -> ActivationFunctionDerivative a -> Network a -> Samples a -> Network a
...
-- functions.
trainUntilErrorBelow :: (Floating (Vector a), Floating a, Product a, Num (Vector a), Ord a, Container Vector a, Num (RealOf a), a ~ RealOf a, Show a) => a -> a -> a -> ActivationFunction a -> ActivationFunctionDerivative a -> Network a -> Samples a -> Network a
trainUntilErrorBelow x rate momentum act = trainUntil (\_ n s -> quadError act n s < x) rate momentum act
